...
export class ProjectIndex {
  ...
  // Call graph (lazily built on first trace/findPaths/findDeadCode call)
  private callGraph: {
  ...
  async reindexFile(relativePath: string): Promise<Result<void, string>> {
    // Invalidate call graph since file changed
    this.invalidateCallGraph();
  ...
  private removeFile(relativePath: string): void {
    // Invalidate call graph since file removed
    this.invalidateCallGraph();
  ...
  // ============================================================================
  // Call Graph Methods (trace, findPaths, findDeadCode)
  // ============================================================================

  /**
   * Invalidate the call graph (called when files change).
   */
  private invalidateCallGraph(): void {
    this.callGraph = null;
  }

  /**
   * Ensure the call graph is built (lazy initialization).
   */
  ...
  private buildCallGraph(): void {
    ...
    for (const [relativePath, tree] of this.indexedFiles) {
      ...
      for (const { symbol, namePath } of flattened) {
        ...
        // Match: identifier followed by ( but not preceded by function/class/etc keywords
   ...
   *
   * @param symbolName - Symbol name to start from
   * @param direction - "forward" for callees, "backward" for callers
   ...
   *
   * @param fromSymbol - Starting symbol name
   * @param toSymbol - Target symbol name
  ...
  findDeadCode(filePattern?: string): Result<DeadCodeResult, string> {
    ...
    for (const node of nodesToAnalyze) {
      ...
      // Determine reason
      const incomingEdges = this.callGraph!.incoming.get(node.id) || [];
      let reason: string;
