...
interface BranchDiffInput {
  base?: string;
  head?: string;
}

export const registerBranchDiff: ToolRegistrar = (server, service) => {
  server.registerTool(
    "branch_diff",
    {
      title: "Branch diff",
    ...
    async (input: BranchDiffInput) => {
      const base = input.base ?? "main";
      const head = input.head ?? "HEAD";
      const result = await service.branchDiff(base, head);
      ...
      if (diff.files.length > 0) {
        ...
        const other = diff.files.filter(
          (f) => !["A", "M", "D"].includes(f.status)
        );
